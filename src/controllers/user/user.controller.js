...
export const register = async (req, res) => {
  ...
  try {
    ...
    const user = await User.scope('withSecretColumns').findOne({ where: { email } });
    if (user) {
...
export const login = async (req, res) => {
  ...
  try {
    ...
    const user = await User.scope('withSecretColumns').findOne({ where: { email: req.body.email } });
    ...
    if (reqPass !== user.password) {
      throw new Error('Incorrect Email Id1/Password');
    }
...
export const changePassword = async (req, res) => {
  ...
  try {
    const { userId } = req.user;
    const user = await User.scope('withSecretColumns').findOne({ where: { id: userId } });
